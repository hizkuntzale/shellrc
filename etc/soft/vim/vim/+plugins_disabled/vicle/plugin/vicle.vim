...
function! Vicle_send_lines(lines)
  if a:lines != ['']
    " Readline воспринимает табуляцию как выбор дополнения
    let l:text = substitute(substitute(join(a:lines, "\n") , "'", "'\\\\''", 'g') , "\t" , "    " , 'g') . "\n"
    call Vicle_up_svars()
...
" Maps for sending
nmap <C-f><C-f> :call Vicle_send_command(g:vicle_normal)<CR>
imap <C-f><C-f> <ESC>:call Vicle_send_command(g:vicle_insert)<CR><Right>
vmap <C-f><C-f> <ESC>:call Vicle_send_command(g:vicle_visual)<CR>
